...
- 主窗口和菜单已支持 `Show/Hide Desktop Pet`、`Show Main Window`、`Add Images`、`Compress More`。
...
- `Done/Issues -> Idle`：用户点击 `Compress More` 或清空任务。
...
- `Compress More` 仅在当前批次完成后显示，可与 `Reveal` 二选一或通过更多菜单承载。
...
| PET-08 | Pet 可开始下一批 | P1 | 完成后支持 Compress More，并保留用户设置 |
...
- 覆盖原图二次确认。
- `Reveal in Finder`、`Retry Failed`、`Compress More`。
...
- Pet 状态模型补齐。
- Pet 操作闭环：Open App、Add Images、Reveal、Retry Failed、Compress More、Hide。
- 主窗口关闭/最小化恢复稳定。
...
1. Pet 完成后底部是否同时显示 `Reveal` 和 `Compress More`，还是优先 `Reveal` 并把 `Compress More` 放到主窗口？
