...
- 透明或极弱背景热区。
- 可选轻微 shadow。
- Mini 在 Issues 状态允许使用异常表情或状态色阴影，但不得显示数字、badge、文字或进度环。
...
| 普通 `issues` | 默认保持当前模式 |
| 全部失败或覆盖取消 | 建议展开 Full |
| Full 非阻塞状态长时间无交互 | P0 不自动收回，由用户手动点击 Collapse 收回；P1 评估自动收回 |
...
- P0 模式下，即便在 Full 状态完成也由用户手动点击 Collapse 收回（不进行自动收回，确保用户有足够时间点击 `Reveal` 和阅读节省结果）。P1 可评估 30 秒无交互自动收回。

原因：自动收回容易打断用户点击 `Reveal` 或阅读节省结果，P0 保持最简单的手动控制体验。
...
- 覆盖取消：展开 Full，显示 `Issues` 和 `Clear List`。
...
为了防止用户拖入文件进行“投喂”时意外触发窗口移动，或在移动窗口时意外触发文件接收：
- **窗口拖动**：仅在本地鼠标按下（mouseDown）并拖拽整个 Pet 窗口时触发。
- **投喂文件**：外部系统的 draggingEntered / performDrop 必须被独立捕获，且仅走文件投喂路径，绝不触发窗口移动逻辑。
...
该状态属于 GUI 展示状态，由 `ImagePetStore` (GUI 层 Store) 或 ViewModel 管理并发布。`DesktopPetWindowController` 仅作为窗口生命周期、尺寸切换、位置恢复与层级的执行者，不维护或擅自修改此 ViewMode 状态机，以保持 WindowController 的职责单一。

### 13.5 窗口尺寸切换锚点

在 Mini 与 Full 模式切换时，`DesktopPetWindowController` 调整窗口大小时必须以 Pet 的视觉中心点为缩放锚点，保持中心点在屏幕上的绝对位置不变，以防窗口尺寸改变时视觉中心发生突兀跳跃。
...
| PET5-08 | Issues 状态切换策略 | P0 | 部分失败只提示；全部失败/权限失败展开 |
| PET5-09 | 右键菜单提供 Show Panel / Open App / Hide | P0 | Mini 无按钮时仍有发现路径 |
