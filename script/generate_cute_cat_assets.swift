...
for spec in specs {
    ...
    for frame in 0..<spec.frames {
        let image = try renderFrame(animation: spec.name, frameIndex: frame, totalFrames: spec.frames)
...
print("Generated CuteCat assets at \(resourcesURL.path)")
...
func renderFrame(animation: String, frameIndex: Int, totalFrames: Int) throws -> CGImage {
    ...
    drawCat(in: context, pose: pose, frameIndex: frameIndex)
...
func drawCat(in context: CGContext, pose: Pose, frameIndex: Int) {
    let fur = cgColor(0.96, 0.58, 0.20, 1.0)
    let furLight = cgColor(1.0, 0.71, 0.33, 1.0)
    let outline = cgColor(0.42, 0.24, 0.13, 1.0)
    let cream = cgColor(1.0, 0.92, 0.76, 1.0)
    let innerEar = cgColor(0.98, 0.62, 0.56, 1.0)
    ...
    drawTail(in: context, fill: fur, stroke: outline, angle: pose.tailAngle)
    ...
    drawHead(in: context, pose: pose, fill: fur, stroke: outline, innerEar: innerEar, dark: dark, cream: cream, blush: blush)
...
func drawTail(in context: CGContext, fill: CGColor, stroke: CGColor, angle: CGFloat) {
    context.saveGState()
...
func drawHead(
    ...
    drawFace(in: context, pose: pose, dark: dark, cream: cream, blush: blush)
    context.restoreGState()
}
...
func drawFace(in context: CGContext, pose: Pose, dark: CGColor, cream: CGColor, blush: CGColor) {
    if pose.cheekPuff > 0 {
